...
임베딩 벡터 기반 경량 분류기:
- MoriGirlVectorClassifier: 1-layer 분류기
...
class MoriGirlVectorClassifier(nn.Module):
    """
    임베딩 벡터 기반 모리걸 스타일 이진 분류기 (경량 버전)
    ...
    def __init__(self, 
        ...
        # 경량 1레이어 분류기 (임베딩 벡터는 이미 추상화된 특징)
...
def get_model_info(model):
    ...
    if hasattr(model, 'hidden_dim'):
        print(f"  - 히든 차원: {model.hidden_dim}")
...
def create_morigirl_model(**kwargs) -> nn.Module:
    ...
    Args:
        **kwargs: 모델 파라미터 (input_dim, hidden_dim, dropout_rate)
    
    Returns:
        모리걸 분류 모델 (1024 → 128 → 1)
    """
    model = MoriGirlVectorClassifier(**kwargs)
    print(f"✅ 모리걸 분류기 생성 (1-layer)")
    return model
